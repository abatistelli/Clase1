...
Solo le otorga préstamos a clientes cuya edad sea mayor a 22 años, se encuentren empleados y tengan más de un año de antigüedad
...
func main() {
	...
	if edad < 23 || trabaja == false || antiguedad < 1 {
		fmt.Println("Usted no puede solicitar un prestamo")
	} else if sueldo > 100000 {
		fmt.Println("Se aprobo su prestamo, y el mismo no le cobrara ningun tipo de interes")
	} else {
		fmt.Println("Se aprobo su prestamo, pero el mismo le cobrara intereses debido a su sueldo")
	}
}
